	...
	PEXCEPTION_POINTERS pExceptionInfo )
{
	...
	//sprintf_s( FileName, MAX_PATH, "CrashDump%2d%2d%2d%2d.dmp", time.wDay, time.wHour, time.wMinute, time.wSecond );
	sprintf( FileName, "CrashDump%2d%2d%2d%2d.dmp", time.wDay, time.wHour, time.wMinute, time.wSecond );
	HANDLE hDumpFile = ::CreateFileA( FileName, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);

	MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(),
		hDumpFile, MiniDumpNormal, &mei, NULL, NULL);
...
int __cdecl WheatyExceptionReport::_tprintf(const TCHAR * format, ...)
{
	...
	retValue = _vsntprintf( szBuff, 1024, format, argptr );
	va_end( argptr );
...
{
	strcpy( m_szMapFilename, pszMapFilename );
}
...
BOOL CExceptionAddressToSrcLine::AddrToSrcLine( const char* pszMapFilename, DWORD dwExceptAddr )
{
	...
	int nLine = 0;
	int nLineStartStrinLen = strlen( "Line numbers" );
	while( fgets( s, 4096, fp) )
	{
		...
		if( strnicmp( s, "Line numbers", nLineStartStrinLen ) == 0 )
		{
			if( !bLineStart )
				bLineStart = TRUE;
			strcpy( szCurSrcFilename, s );
			continue;	
		}
		if( !bLineStart )
			continue;

		int nRecordCount = nLen/20;
		for( int i = 0; i < nRecordCount; i++)
		{
				...
				dwExceptAddr-dwAddr < dwMin )
			{
				...
				strcpy( m_szSrcFileDesc, szCurSrcFilename );
			}
		}
	...
	strcpy( szSrcFilename, m_szSrcFileDesc );
	int nLen = strlen( szSrcFilename );
	for( int i = 0; i < nLen; i++ )
	{
		if( szSrcFilename[i] == '(' )
		{
			strcpy( m_szSrcFileDesc, &szSrcFilename[i+1] );
		}
	}
